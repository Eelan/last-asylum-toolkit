...
test('Corbeau: completed phases apply only to the first upgrade', () => {
  ...
  assert.equal(full.fruit - partial.fruit, 2 * cost.fruitPerAction);
  assert.equal(calculateRavenProgression(5, 6).fruit, 5 * 2250);
  assert.equal(calculateRavenProgression(5, 6, 4).fruit, 2250);
  assert.equal(calculateRavenProgression(1, 2).fruit, 4000);
  assert.equal(getRavenUpgradeCost(1).stepCount, 4);
  assert.equal(calculateRavenProgression(1, 2, 1).fruit, 3000);
  assert.equal(calculateRavenProgression(51, 52).fruit, 135000);
  assert.equal(getRavenUpgradeCost(51).stepCount, 6);
  assert.equal(calculateRavenProgression(51, 52, 2).fruit, 90000);
  assert.equal(getRavenUpgradeCost(106).stepCount, 13);
  assert.equal(calculateRavenProgression(106, 107, 12).fruit, 15000);
  assert.equal(calculateRavenProgression(26, 27).fruit, null);
  assert.equal(calculateRavenProgression(26, 27).fruitKnown, false);
