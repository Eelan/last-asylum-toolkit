...
export function renderDuelPage(tool) {
 ...
 <div class="calc-grid"><section class="panel"><div class="form-grid">
  ...
  <label>${renderResourceLabel('antitoxin', translate('antitoxin'))}<input id="d-a" inputmode="numeric" value="0"></label>
  <label>${renderResourceLabel('recruitments', translate('recruits'))}<input id="d-r" inputmode="numeric" value="0"></label>
  <label>${renderResourceLabel('ur-omni-shards', translate('ur_omni_shards'))}<input id="d-u" inputmode="numeric" value="0"></label>
  <label>${renderResourceLabel('ssr-omni-shards', translate('ssr_omni_shards'))}<input id="d-s" inputmode="numeric" value="0"></label>
  <label>${renderResourceLabel('sr-omni-shards', translate('sr_omni_shards'))}<input id="d-sr" inputmode="numeric" value="0"></label>
  <label>${renderResourceLabel('skill-badges', translate('skill_badges'))}<input id="d-b" inputmode="numeric" value="0"></label>
 </div></section>
 ...
 <section class="panel table-panel"><h3>${translate('consumption_plan')}</h3><div class="table-wrap"><table><thead><tr><th>${translate('resource')}</th><th>${translate('stock')}</th><th>${translate('use')}</th><th>${translate('keep')}</th><th>${translate('duel_points')}</th></tr></thead><tbody id="d-plan"></tbody></table></div><p class="form-note">${translate('duel_plan_note')}</p></section>`;
  ...
  const calculate = () => {
    ...
    const iconResources = { ur: 'ur-omni-shards', ssr: 'ssr-omni-shards', recruits: 'recruitments', sr: 'sr-omni-shards', badges: 'skill-badges', antitoxin: 'antitoxin' };
    $('#d-plan').innerHTML = plan.resources.map(resource => `<tr><td>${renderResourceLabel(iconResources[resource.key], labels[resource.key])}</td><td>${formatNumber(resource.stock)}</td><td>${formatNumber(resource.use)}</td><td>${formatNumber(resource.keep)}</td><td>${formatNumber(resource.points)}</td></tr>`).join('');
  };
  // #endregion
