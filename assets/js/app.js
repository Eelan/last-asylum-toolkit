...
function renderDuelPage(t) {
  $('#view').innerHTML = renderPageHeader(t) + `
 <div class="calc-grid"><section class="panel">
  <div class="form-grid">
 ...
 <section class="panel table-panel"><h3>${translate('consumption_plan')}</h3><div class="table-wrap"><table><thead><tr><th>${translate('resource')}</th><th>${translate('stock')}</th><th>${translate('use')}</th><th>${translate('keep')}</th><th>${translate('duel_points')}</th></tr></thead><tbody id="d-plan"></tbody></table></div><p class="form-note">${translate('duel_plan_note')}</p></section>`;
  ...
  const settingFields = { 'd-target': 'lat-duel-target', 'd-bonus': 'lat-duel-research-bonus' };
  Object.entries(settingFields).forEach(([id, key]) => {
    try {
  ...
  const calculate = () => {
    ...
    const resources = [
      { id: 'd-u', label: translate('ur_omni_shards'), points: d.urShardPoints, unit: 1 },
      { id: 'd-s', label: translate('ssr_omni_shards'), points: d.ssrShardPoints, unit: 1 },
      { id: 'd-r', label: translate('recruits'), points: d.recruitPoints, unit: 1 },
      { id: 'd-sr', label: translate('sr_omni_shards'), points: d.srShardPoints, unit: 1 },
      { id: 'd-b', label: translate('skill_badges'), points: d.skillBadgePoints, unit: 1 },
      { id: 'd-a', label: translate('antitoxin'), points: 1, unit: d.antitoxinUnit }
    ...
    const availableBasePoints = resources.reduce((total, resource) => total + Math.floor(resource.stock / resource.unit) * resource.points, 0);
    const availablePoints = Math.floor(availableBasePoints * multiplier);
    ...
    if (remainingBasePoints > 0) {
      ...
      if (candidate) candidate.useUnits += 1;
    }

    const plannedBasePoints = resources.reduce((total, resource) => total + resource.useUnits * resource.points, 0);
    const plannedPoints = Math.floor(plannedBasePoints * multiplier);
    const missingPoints = target > 0 ? Math.max(0, target - plannedPoints) : 0;
    $('#d-total').textContent = formatNumber(availablePoints);
    $('#d-target-result').textContent = formatNumber(target);
    $('#d-missing').textContent = formatNumber(missingPoints);
    $('#d-plan').innerHTML = resources.map(resource => {
      const use = resource.useUnits * resource.unit;
      const keep = resource.stock - use;
      const points = Math.floor(resource.useUnits * resource.points * multiplier);
      return `<tr><td>${resource.label}</td><td>${formatNumber(resource.stock)}</td><td>${formatNumber(use)}</td><td>${formatNumber(keep)}</td><td>${formatNumber(points)}</td></tr>`;
    }).join('');
  };
  bindPersistentStocks({
    'd-a': 'antitoxin',
